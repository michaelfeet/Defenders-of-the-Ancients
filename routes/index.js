...
router.get('/oauth2callback', passport.authenticate(
  'google',
  {
    successRedirect : '/heros', // UPDATE THIS, where do you want the client to go after you login 
    failureRedirect : '/heros' //  UPDATE THIS, where do you want the client to go if login fails
  }
));
...
router.get('/logout', function(req, res){
  req.logout(function(){ //< - req.logout comes from passport, and what it does is destroys the cookie keeping track of the user!
    res.redirect('/heros') // <---- UPDATE THIS TO WHERE YOU WANT THE USER TO GO AFTER LOGOUT
  })
})
